import formatDate, { formatDateForActivity, localMoment } from 'dateFormatter';
...
describe('dateFormatter', () => {
  ...
  it('formats time yesterday for grouped activity', () => {
    ...
    expect(formatDateForActivity(date, true)).to.equal('17:00');
  });

  it('formats time yesterday', () => {
    let date = localMoment().subtract(1, 'day').hour(17).minute(0).toDate();

    expect(formatDate(date)).to.equal('Yesterday 17:00');
  });
  ...
  it('gives precedence to shorter format', () => {
    ...
    expect(formatDate(date, now)).to.equal('Yesterday 23:59');
  });
});
