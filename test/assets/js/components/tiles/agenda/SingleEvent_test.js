...
describe('SingleEvent', () => {
  ...
  it('renders a single time when start == end', () => {
    ...
    const html = shallowAtMoment(<SingleEvent {...props} event={event} />, now);
  ...
  it('renders the weekday for the day after tomorrow', () => {
    ...
    const event0 = shallowAtMoment(<SingleEvent {...props} event={content.items[0]} />, now);
    extractDate(event0).should.equal('Sat 12:00');

    const event1 = shallowAtMoment(<SingleEvent {...props} event={content.items[1]} />, now);
  ...
  it('renders an event with multiple locations when small', () => {
    ...
    items.at(2).text().includes('Location, Location?, Location!').should.equal(true);
