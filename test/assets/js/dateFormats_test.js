...
describe('dateFormats', () => {
  ...
  it('says today when asked', () => {
    let date = localMoment().hour(9).minute(0).toDate();
    expect(dateFormats.forNewsItem(date, undefined, { })).to.equal('09:00');
    expect(dateFormats.forNewsItem(date, undefined, { printToday: true })).to.equal('Today 09:00');
  });
  ...
  it('only shows weekday when onlyWeekday is true', () => {
    let date = localMoment().year(2016).dayOfYear(210).hour(17).minute(30).toDate();
    let now = localMoment().year(2016).dayOfYear(200).toDate();
    expect(dateFormats.forNewsItem(date, now)).to.equal('Thu 28 Jul, 17:30');
    expect(dateFormats.forNewsItem(date, now, { onlyWeekday: true })).to.equal('Thu 17:30');
  });
});
