...
class ActivityTest extends BaseSpec {
  ...
  "ActivityRender" should {
    "not output textAsHtml in Json writes if text doesn't include Markdown characters" in {
      val provider: ActivityProvider = ActivityProvider("id", false)
      ...
      val activityWithNoMarkdownText = Activity("id", "providerId", "type", "title", textWithNoMarkdownText, None, None, DateTime.now, DateTime.now, Usercode("cusjau"), true, true)
      ...
      val activityWithNewLines = Activity("id", "providerId", "type", "title", textWithNewLines, None, None, DateTime.now, DateTime.now, Usercode("cusjau"), true, true)
      ...
      val activityWithSpecialCharacters = Activity("id", "providerId", "type", "title", textWithSpecialCharacters, None, None, DateTime.now, DateTime.now, Usercode("cusjau"), true, true)
      ...
      val activityThatIsntMarkdown = Activity("id", "providerId", "type", "title", textThatIsntMarkdown, None, None, DateTime.now, DateTime.now, Usercode("cusjau"), true, true)
