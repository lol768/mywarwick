...
const browserifyOptions = (entries) => ({
  entries,
...
gulp.task('scripts', [], () => {
  _.map(SCRIPTS, (entries, output) => {
    const b = browserifyFlags(browserify(browserifyOptions(entries)));
...
function exportAssetModule(name, taskName, baseDir, extraExtensions) {
  gulp.task(taskName, () => {
    ...
    const baseExtensions =
      ['otf', 'eot', 'woff', 'woff2', 'ttf', 'js', 'js.map', 'gif', 'png', 'jpg', 'svg'];
...
function getFontAwesomeVersion() {
  return new Promise((resolve, reject) => {
    lessCompiler.render(
      '@import "node_modules/id7/less/font-awesome/variables.less"; @{fa-version} { a: a; }',
      {},
      (e, output) => {
        const version = output.css.match(/"([0-9\.]+)"/)[1];
        if (version) {
          resolve(version);
        } else {
          reject();
...
gulp.task('manifest', ['scripts', 'styles'], () => {
  if (PRODUCTION) {
      ...
      .then((fontAwesomeVersion) => (
        cacheableAssets
          .pipe(rename((p) => {
            ...
            if (result.basename === 'fontawesome-webfont') {
              result.extname += `?v=${fontAwesomeVersion}`;
            }
