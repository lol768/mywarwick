...
import services.ActivityError.{InvalidJSON, InvalidProviderId}
import services._
...
class IncomingActivitiesController @Inject()(
  ...
  def postActivity(providerId: String) = APIAction { implicit request =>
    postItem(providerId, shouldNotify = false)
  }

  def postNotification(providerId: String) = APIAction { implicit request =>
    postItem(providerId, shouldNotify = true)
  }

  def postItem(providerId: String, shouldNotify: Boolean)(implicit request: AuthenticatedRequest[AnyContent]): Result =
        ...
        case Some(publisherId) =>
          request.context.user.map { user =>
            if (publisherService.getRoleForUser(publisherId, user.usercode).can(CreateAPINotifications)) {
              json.validate[IncomingActivityData].map { data =>
