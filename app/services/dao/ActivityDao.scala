    ...
    val activities = SQL(
      s"""
        SELECT
          ...
          ACTIVITY_TAG_VALUE.DISPLAY_VALUE AS TAG_DISPLAY_VALUE
        FROM ACTIVITY
          LEFT JOIN ACTIVITY_TAG ON ACTIVITY_TAG.ACTIVITY_ID = ACTIVITY.ID
          LEFT JOIN PROVIDER ON PROVIDER.ID = ACTIVITY.PROVIDER_ID
          LEFT JOIN ACTIVITY_TAG_VALUE ON
            ACTIVITY_TAG_VALUE.TAG_NAME = ACTIVITY_TAG.NAME
          AND ACTIVITY_TAG_VALUE.VALUE = ACTIVITY_TAG.VALUE
