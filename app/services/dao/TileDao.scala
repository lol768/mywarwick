...
class TileDaoImpl @Inject()() extends TileDao {
  ...
  override def getTilesByIds(usercode: String, ids: Seq[String])(implicit c: Connection): Seq[UserTile] =
    if (ids.isEmpty)
      ...
      SQL("SELECT TYPE, DEFAULT_SIZE, FETCH_URL, TILE_POSITION, TILE_SIZE, CREATED_AT, UPDATED_AT FROM USER_TILE JOIN TILE ON TILE.TYPE = TILE_TYPE WHERE TYPE IN ({ids}) AND USERCODE = {usercode}")
        .on(
          'usercode -> usercode,
          'ids -> ids
        )
  ...
  override def getTilesForUser(usercode: String)(implicit c: Connection): Seq[UserTile] =
    SQL("SELECT TYPE, DEFAULT_SIZE, FETCH_URL, TILE_POSITION, TILE_SIZE, CREATED_AT, UPDATED_AT FROM USER_TILE JOIN TILE ON TILE.TYPE = TILE_TYPE WHERE USERCODE = {usercode} ORDER BY TILE_POSITION ASC")
  ...
  //TODO: define collection of default tiles, and return them here
    SQL("SELECT TYPE, DEFAULT_SIZE, FETCH_URL, TILE_POSITION, TILE_SIZE, CREATED_AT, UPDATED_AT FROM USER_TILE JOIN TILE ON TILE.TYPE = TILE_TYPE")
      .as(userTileParser.*)
  ...
  def userTileParser: RowParser[UserTile] = {
    get[String]("TYPE") ~
      ...
      case tileType ~ defaultSize ~ fetchUrl ~ position ~ size ~ createdAt ~ updatedAt =>
        UserTile(
          Tile(tileType, TileSize.withName(defaultSize), fetchUrl),
